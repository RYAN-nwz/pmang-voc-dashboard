...
def get_gspread_client():
    try:
        ...
        try:
            ...
            # 로컬 환경 (json 파일)
            # [수정] 스크립트의 실제 위치를 기준으로 service_account.json 경로를 설정
...
def load_voc_data(spreadsheet_id: str) -> pd.DataFrame:
    ...
    'YY-MM' (예: 25-09) 형식의 모든 시트를 읽어와 성능과 확장성을 모두 확보합니다.
    """
    ss = open_sheet(spreadsheet_id)
    if not ss:
        return pd.DataFrame()
    try:
        ...
        if not monthly_sheet_titles:
            # [수정] '25-09' 시트가 있는데도 데이터를 못 읽는 경우를 대비해, 기존 일별 시트도 읽도록 임시 허용
            st.warning("데이터 구조 업그레이드 중... 'YY-MM' 형식의 월별 시트를 찾지 못했습니다. 기존 일별 시트를 읽습니다.")
            for ws in all_worksheets:
                if ws.title.lower() in ["sheet1", "template", "mapping", "user_management"] or re.match(r'^\d{2}-\d{2}$', ws.title):
                    continue
                try:
                    rows = ws.get_all_records()
                    if rows:
                        for r in rows:
                            r["날짜"] = ws.title # 일별 시트 제목(YYMMDD)을 날짜로 사용
            ...
            for sheet_title in monthly_sheet_titles:
                try:
                    ...
                    rows = ws.get_all_records()
                    if rows:
                        # [수정] 월별 시트는 이미 '날짜' 컬럼이 있으므로 ws.title을 덮어쓰지 않음
                        all_data.extend(rows)
                except gspread.WorksheetNotFound:
                    continue
                except Exception as e:
        ...
        # '날짜' 컬럼이 있는지 확인 (월별/일별 시트 데이터가 혼재된 경우)
        if "날짜" not in df.columns:
            st.error("데이터에 '날짜' 컬럼이 없습니다. (데이터 소스 확인 필요)")
            return pd.DataFrame()
