...
def get_gspread_client():
    try:
        ...
        try:
            ...
            # 로컬 개발 편의: service_account.json
            script_dir = os.path.dirname(os.path.abspath(__file__))
            service_file = os.path.join(script_dir, "service_account.json")
...
def load_voc_data(spreadsheet_id: str) -> pd.DataFrame:
    """
    Google Sheets에서 '월별(YY-MM)' 시트의 VOC 데이터를 불러온다.
    각 시트는 하나의 월 데이터를 포함하고, 데이터 내부 '날짜' 컬럼은 YYMMDD 형태라고 가정한다.
    """
    ss = open_sheet(spreadsheet_id)
    if not ss:
        return pd.DataFrame()
    try:
        ...
        for ws in ss.worksheets():
            title = ws.title
            # 'YY-MM' 시트만 읽기 (예: 25-09, 25-10, 26-01 ...)
            if re.match(r'^\d{2}-\d{2}$', title) is None:
                # 내부 관리용/템플릿 등은 스킵
                continue
            try:
                rows = ws.get_all_records()
                if rows:
                    # 월별 시트는 본문 데이터에 '날짜'(YYMMDD) 컬럼이 존재해야 함
        ...
        # 필수 컬럼 확인
        required = ["날짜", "접수 카테고리", "상담제목", "문의내용", "taglist"]
...
def main():
    ...
    default_start = today_kst - timedelta(days=6)
    ...
    with st.sidebar:
        ...
        if voc_df.empty:
            ...
            # 초기 date_range 세팅 (세션 보존)
            if "date_range" not in st.session_state:
                start_init = max(min_d, default_start)
                end_init = max_d
                st.session_state.date_range = (start_init, end_init)

            # 최근 N일 버튼
            def set_range(days):
                start = max(max_d - timedelta(days=days-1), min_d)
            ...
            # date_input에 안전한 value 주입
            cur_start, cur_end = st.session_state.date_range
            cur_start = max(cur_start, min_d)
            cur_end = min(cur_end, max_d)
            if cur_start > cur_end:
                cur_start = max(min_d, max_d - timedelta(days=6))
                cur_end = max_d
            st.session_state.date_range = (cur_start, cur_end)
    ...
    # =============================
    # 본문: 탭(분석/검색/결제리포트) + 어드민(하단)
    # =============================
    # 조회 기간 및 데이터 적용
    if voc_df.empty or filtered.empty:
        st.warning("표시할 데이터가 없습니다. 필터/기간을 조정하세요.")
        view_df = pd.DataFrame()
        tabs = st.tabs(["📊 카테고리 분석", "🔍 키워드 검색", "💳 결제/인증 리포트"])  # 빈 탭은 유지
    else:
        # date_input이 tuple(date,date) 보장
        start_dt: date = st.session_state.date_range[0]
        end_dt: date = st.session_state.date_range[1]
    ...
    # 탭 컨텐츠
    tab_main, tab_search, tab_payment = tabs
    ...
    with tab_main:
        if not view_df.empty:
            ...
            with st.container(border=True):
                ...
                with c1:
                    default_cats = top5.index.tolist() if len(top5) > 0 else []
                    selected_cats = st.multiselect("L2 태그 필터:", options=all_cats, default=default_cats)
                with c2:
    ...
    with tab_search:
        ...
        if last_keyword and not view_df.empty:
            ...
            if keywords:
                ...
                if r.empty:
                    ...
                    with st.container(border=True):
                        st.header("검색 결과 추이")
                        st.plotly_chart(create_trend_chart(r, (start_dt, end_dt), f"'{last_keyword}' 일자별 발생 추이"), use_container_width=True)
    ...
    # =============================
    # 하단: 어드민 멤버 관리 (항상 하단에만 표시)
    # =============================
    if is_admin:
        st.markdown("---")
        st.subheader("🛡️ 어드민 멤버 관리")
        users_df_latest = fetch_users_table(spreadsheet_id)  # 최신 정보로 로드
