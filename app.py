    ...
    try:
        ...
        try:
            ...
            if os.path.exists(SERVICE_ACCOUNT_FILE):
                ...
                st.error(f"인증 정보를 찾을 수 없습니다: '{SERVICE_ACCOUNT_FILE}' 경로를 확인하세요.")
                st.stop()
...
def _clamp_date_range(rng, min_d: date, max_d: date):
    """(start, end)을 [min_d, max_d]로 클램프하고 start<=end 보장."""
    if not isinstance(rng, (list, tuple)) or len(rng) != 2:
        return (max(min_d, max_d - timedelta(days=6)), max_d)
    start, end = pd.to_datetime(rng[0]).date(), pd.to_datetime(rng[1]).date()
...
def load_voc_data(spreadsheet_id: str) -> pd.DataFrame:
    """
    Google Sheets에서 'YY-MM' (예: 25-09, 25-10) 형식의 모든 시트를 읽어옵니다.
    월별 시트가 없다면 과거 일별 시트(YYMMDD)도 폴백으로 읽습니다.
    """
    ss = open_sheet(spreadsheet_id)
    if not ss:
        return pd.DataFrame()
    try:
        ...
        if monthly_sheet_titles:
            st.sidebar.info(f"데이터 로딩: 월별 시트 {len(monthly_sheet_titles)}개")
            for sheet_title in monthly_sheet_titles:
                try:
                    ws = ss.worksheet(sheet_title)
                    rows = ws.get_all_records()
                    if rows:
                        # 월별 시트는 행마다 '날짜' 컬럼이 존재한다고 가정
            ...
            for ws in all_worksheets:
                ...
                try:
                    ...
                    if rows:
                        for r in rows:
                            r["날짜"] = title  # 시트명(YYMMDD)을 날짜로 사용
                        all_data.extend(rows)
                except Exception:
                    continue
        ...
        if "날짜" not in df.columns:
            st.error("데이터에 '날짜' 컬럼이 없습니다. (수집 스크립트 확인 필요)")
            return pd.DataFrame()
    ...
    with st.sidebar:
        ...
        if voc_df.empty:
            ...
            if not selected:
                ...
                for opt in selected:
                    if " for kakao" in opt:
                        ...
                        conds.append((voc_df["게임"] == game_name) & (voc_df["플랫폼"] == "for kakao"))
                    else:
                        parts = opt.rsplit(" ", 1)
                        game_name = parts[0]
                        platform = parts[1] if len(parts) > 1 else None
                        if platform:
                            conds.append((voc_df["게임"] == game_name) & (voc_df["플랫폼"] == platform))
                        else:
                            conds.append(voc_df["게임"] == game_name)
                if conds:
            ...
            if filtered.empty:
                ...
                def recent_n_days(n):
                    start = max_d - timedelta(days=n-1)
                    if start < min_d:
                        start = min_d
                    # 버튼으로 설정하는 값도 min/max로 클램프
                    st.session_state["date_range_value"] = (start, max_d)

                c1, c2 = st.columns(2)
                with c1:
                    st.button("최근 7일", on_click=recent_n_days, args=(7,), use_container_width=True)
                with c2:
                    st.button("최근 30일", on_click=recent_n_days, args=(30,), use_container_width=True)

                # 기본값 계산 (세션이 없거나 범위를 벗어나면 보정)
                default_range = (max(min_d, max_d - timedelta(days=6)), max_d)
                current = st.session_state.get("date_range_value", default_range)
                current = _clamp_date_range(current, min_d, max_d)
                st.session_state["date_range_value"] = current

                # date_input: key를 date_range_value로 고정해서 위 경고/충돌 방지
