...
def get_gspread_client():
    try:
        ...
        # 배포(Secrets) 우선, 실패 시 로컬 파일 fallback
        creds = None
        try:
            ...
            if os.path.exists(SERVICE_ACCOUNT_FILE):
                ...
                raise RuntimeError(f"인증 정보를 찾을 수 없습니다. Secrets 또는 {SERVICE_ACCOUNT_FILE} 확인")
        return gspread.authorize(creds)
    except Exception as e:
        st.error("Google 인증 실패: 인증 구성을 확인하세요.")
        st.exception(e)
        return None
...
def load_voc_data(spreadsheet_id: str) -> pd.DataFrame:
    """
    월별 시트(YY-MM, YYYY-MM, YY_MM) 위주로 읽고,
    없으면 구버전(일별 YYMMDD) 시트를 백업으로 읽는다.
    """
    ss = open_sheet(spreadsheet_id)
    if not ss:
        return pd.DataFrame()
    try:
        ...
        # 월별 시트 패턴: 25-09, 2025-09, 25_11 등 허용
        monthly_titles = []
        for ws in all_worksheets:
            title = ws.title.strip()
            if re.match(r'^\d{2,4}[-_]\d{2}$', title):
                monthly_titles.append(title)

        if monthly_titles:
            st.sidebar.info(f"월별 데이터 로딩: {len(monthly_titles)}개 시트")
            for title in monthly_titles:
                try:
                    ws = ss.worksheet(title)
                    rows = ws.get_all_records()
                    if rows:
                        # 월별 시트는 각 행에 '날짜'(YYMMDD)가 들어있다고 가정
                        all_data.extend(rows)
                except Exception as e:
                    st.warning(f"'{title}' 시트 로딩 중 오류: {e}")
        else:
            # 백업: 구버전(일자별 시트) 읽기
            st.sidebar.warning("월별 시트를 찾지 못해, 일별 시트를 백업 방식으로 읽습니다.")
            for ws in all_worksheets:
                title = ws.title.strip()
                if title.lower() in ["sheet1", "template", "mapping", "user_management"]:
                    continue
                if re.match(r'^\d{6,8}$', title):  # 250928 또는 20250928 등
                    try:
                        rows = ws.get_all_records()
                        if rows:
                            for r in rows:
                                r["날짜"] = title  # 시트 제목을 날짜로 사용
                            all_data.extend(rows)
                    except Exception:
                        continue
        ...
        if "날짜" not in df.columns:
            st.error("데이터에 '날짜' 컬럼이 없습니다. (수집 스크립트/시트 구조 확인)")
            return pd.DataFrame()
        ...
        # 날짜 파싱: YYMMDD 또는 YYYYMMDD 모두 허용
        # 먼저 YYMMDD 시도, 실패분은 YYYYMMDD 재시도
        dt1 = pd.to_datetime(df["날짜"], format="%y%m%d", errors="coerce")
        need_retry = dt1.isna()
        if need_retry.any():
            dt2 = pd.to_datetime(df.loc[need_retry, "날짜"], format="%Y%m%d", errors="coerce")
            dt1.loc[need_retry] = dt2
...
def create_donut_chart(data, title, group_by='L2 태그'):
    ...
    fig.update_layout(title_text=f"<b>{title}</b>", showlegend=False, height=300,
...
def main():
    ...
    default_range = (today_kst - timedelta(days=6), today_kst)
    ...
    with st.sidebar:
        ...
        if voc_df.empty:
            ...
            with col1:
                st.button("최근 7일", on_click=set_range, args=(7,), use_container_width=True)
            with col2:
                st.button("최근 30일", on_click=set_range, args=(30,), use_container_width=True)

            if "date_range" not in st.session_state:
                # 최초 진입 시 최근 7일
                st.session_state.date_range = (
                    max(min_d, max_d - timedelta(days=6)),
                    max_d
                )

            # 범위가 시계열 밖이면 보정
            dr = st.session_state.date_range
            start_guard = max(min_d, dr[0])
            end_guard = min(max_d, dr[1])
            if start_guard > end_guard:
                start_guard, end_guard = max_d - timedelta(days=6), max_d
            st.session_state.date_range = (start_guard, end_guard)
    ...
    if filtered.empty or not (isinstance(date_range, (list, tuple)) and len(date_range) == 2):
        ...
        if view_df.empty:
            ...
            with tab_main:
                ...
                with st.container(border=True):
                    ...
                    if selected_cats and selected_sentiments:
                        ...
                        st.dataframe(
                            show_df[["구분","날짜","게임","L1 태그","L2 태그","상담제목","문의 내용","GSN(USN)","기기정보","감성"]],
            ...
            with tab_search:
                ...
                last_keyword = st.session_state.get("last_search_keyword", "").strip()
                if last_keyword:
                    ...
                    if keywords:
                        ...
                        if r.empty:
                            ...
                            with st.container(border=True):
                                ...
                                st.dataframe(
                                    disp_r[["구분","날짜","게임","L2 태그","상담제목","문의 내용","GSN(USN)","기기정보","감성"]],
            ...
            with tab_payment:
                ...
                if payment_auth_df.empty:
                    ...
                    c1, c2 = st.columns(2)
                    with c1:
                        st.plotly_chart(create_trend_chart(payment_auth_df, date_range, "결제/인증 관련 VOC 발생 추이"),
                                        use_container_width=True)
                    with c2:
                        l2_counts_payment = payment_auth_df["L2 태그"].value_counts().nlargest(10).sort_values(ascending=True)
                        fig_l2_payment = px.bar(
                            l2_counts_payment,
                            x=l2_counts_payment.values, y=l2_counts_payment.index, orientation='h',
                    ...
                    with st.container(border=True):
                        ...
                        st.dataframe(
                            disp_payment[["구분","날짜","게임","L1 태그","L2 태그","상담제목","문의 내용","GSN(USN)","기기정보","감성"]],
                            use_container_width=True, height=500
                        )

    # --- 4) 어드민 탭 (데이터 유무와 상관없이 표시)
    if is_admin:
        st.markdown("---")
