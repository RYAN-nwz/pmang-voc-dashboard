...
COLOR_NEGATIVE = "#FF6B6B" # 심각, 부정 (Plotly chart에서 사용)
...
# 🚨 [재통합된 함수] 게임별 전일 VOC 핵심 요약 및 샘플 추출
...
def create_donut_chart(data, title, group_by='L2 태그'):
    ...
    color_sequence = [COLOR_PRIMARY, COLOR_NEGATIVE, COLOR_ACCENT, '#4D94B2', '#06D6A0', '#A3B3C2']
...
def main():
    ...
    st.markdown(f"""
        <style>
            ...
            /* metric value 폰트 크기 및 색상 조정 */
            [data-testid="stMetricValue"] {{
                font-size: 1.8rem; 
                font-weight: 900;
                color: {COLOR_PRIMARY};
            }}
            /* metric label 폰트 크기 및 색상 조정 */
            ...
            /* Expander 헤더 스타일 조정 (전일 VOC 상세 분석) */
            .stExpander {{
    ...
    with tabs[0]:
        ...
        with st.container():
            st.markdown(f'<div class="main-card">', unsafe_allow_html=True)
            st.markdown(f'<h3 style="color:{COLOR_DARK}; font-weight: bold;">📑 VOC 원본 데이터 (L2 태그 기준)</h3>', unsafe_allow_html=True)
    ...
    with tabs[1]:
        ...
        with st.container():
            st.markdown(f'<div class="main-card">', unsafe_allow_html=True)
            st.markdown(f'<h3 style="color:{COLOR_DARK}; font-weight: bold;">키워드 검색 도구</h3>', unsafe_allow_html=True)
        ...
        if last_keyword:
            ...
            if keywords:
                ...
                if r.empty:
                    ...
                    with st.container():
                        st.markdown(f'<div class="main-card">', unsafe_allow_html=True)
                        st.markdown(f'<h3 style="color:{COLOR_DARK}; font-weight: bold;">검색 결과 추이</h3>', unsafe_allow_html=True)
                    ...
                    with st.container():
                        st.markdown(f'<div class="main-card">', unsafe_allow_html=True)
                        st.markdown(f'<h3 style="color:{COLOR_DARK}; font-weight: bold;">관련 VOC 목록</h3>', unsafe_allow_html=True)
                        for c in r.columns:
                            r[c] = r[c].astype(str)
                        st.download_button(
                            "📥 CSV 다운로드",
                    ...
                    with st.container():
                        st.markdown(f'<div class="main-card">', unsafe_allow_html=True)
                        st.markdown(f'<h3 style="color:{COLOR_DARK}; font-weight: bold;">연관 키워드 워드클라우드</h3>', unsafe_allow_html=True)
    ...
    with tabs[2]:
        ...
        if payment_auth_df.empty:
            ...
            with st.container():
                st.markdown(f'<div class="main-card">', unsafe_allow_html=True)
                st.markdown(f'<h3 style="color:{COLOR_DARK}; font-weight: bold;">📑 관련 VOC 원본 데이터</h3>', unsafe_allow_html=True)
